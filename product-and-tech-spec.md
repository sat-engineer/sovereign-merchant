...
1. `GET /api/status`
   ...
   - **Returns:**
     ...
     ```json
     {
       ...
       "reconciliation": {
         "mode": "deposit",
         "lastSyncAt": "2025-10-31T18:00:00Z",
         "lastStatus": "ok"
       },
...
6. Upon successful OAuth callback, UI shows “✅ Connected to QuickBooks: _Company Name_ (via your Intuit app)” and links back to the doc panel for future reference.
...
**Reconciliation Execution:** 4. Build the AccountingProvider payload (see Section 13.4) with the appropriate amount based on reconciliation status. 5. Invoke `QuickBooksOnlineProvider.reconcile(...)` which talks directly to Intuit’s REST APIs—no BTCPay plugin calls. 6. Write success/failure + reconciliation details to SQLite. 7. Mark webhook event as processed (prevents duplicate processing).
...
for you / BTCNYC contributors / future you: 1. prereqs: Node LTS, pnpm (or npm), Docker, SQLite 2. install:
